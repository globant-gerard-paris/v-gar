...
public class StoreBatch {
	...
	public ItemReader<Store> reader() {
		...
		// TODO is commented the following line because is still TBD the way that will take the file
		// stores.
	...
	public Job importerStores(JobBuilderFactory jobs, Step s1) {
		return jobs.get("importerStores").incrementer(new RunIdIncrementer()).flow(s1).end()
				.build();
	}
